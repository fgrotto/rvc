%% multipoint_trajectory_acc_cont.m
...
function [q,d_q,dd_q] = multipoint_trajectory_acc_cont(q_d, t_d, d_qi, d_qf, Ts)
    assert(length(q_d) == length(t_d), "they should be pairs of (q_k,t_k)");
    n = length(q_d);

    % Calculate v vector according to the linear system Ad_q=c
    v(1) = d_qi;
    v(n) = d_qf;
